...
class PunchAnOrk {
  ...
  playerRollAttackDice() {
    return this.playerHand.map(_element => this.rollD6());
  }

  computerRollAttackDice() {
    return this.computerHand.map(_element => this.rollD6());
  }

  displayDiceRoll(dice) {
    return dice.map(element => String.fromCharCode(9855 + element)).join(' ');
  }
...
function game() {
  ...
  currentGame.playerHand = currentGame.playerRollAttackDice();
  currentGame.computerHand = currentGame.computerRollAttackDice();
  
  console.log(`Your Roll: ${currentGame.displayDiceRoll(currentGame.playerHand)}`);
  console.log(`Your Weapon Skill: ${currentGame.playerWeaponSkill}+`);
  console.log('');
  console.log(`The Ork's Roll: ${currentGame.displayDiceRoll(currentGame.computerHand)}`);
