  ...
  computerAction() {
    ...
    (this.playerUnitHand.length === 0)) {
      if (this.computerHand.includes(6)) {
        ...
        console.log(`Unit Health: ${this.playerUnitWounds}`); //Fix Negative Health
        this.pause("");
      } else {
        this.removeDiceFromHand(this.computerHand, this.findNonSix(this.computerHand));       
    ...
    while (anotherRound === 'y' && currentGame.computer.stats.wounds > 0) {
        ...
        while (currentGame.playerUnitWounds > 0) {
          ...
          if (currentGame.playerUnitHand.length > 0) {
            ...
            while (!currentGame.playerUnitHand.includes(+chosenDice)) {
              chosenDice = readlineSync.question('Invalid Selection. Choose a Dice.');
            }

            //chosen dice is removed from the hand
            currentGame.removeDiceFromHand(currentGame.playerUnitHand, chosenDice);
            ...
            if (diceAction === 'Attack') {
              currentGame.playerUnitAttack(chosenDice);
            } else if (diceAction === 'Parry') {
              currentGame.playerUnitParry(chosenDice);
          ...
          if (currentGame.playerUnitWounds <= 0) {
            anotherRound = 'n';
            break;
          }
      ...
      if (currentGame.playerUnitWounds >= 0 && currentGame.computer.stats.wounds > 0) {
        currentGame.pause("You Ran Coward!");
        break;
      } else if (currentGame.playerUnitWounds <= 0) {
